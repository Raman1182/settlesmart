        ...
        <Card className="h-full">
            ...
            <CardContent>
                <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                {debtsOwedByUser.map((s, i) => (
                                    <div key={`owed-by-${i}`} className="flex items-center">
                                        ...
                                        <div className="ml-auto font-medium text-red-500">{formatCurrency(s.amount)}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                            ...
                            <div className="space-y-3">
                                {debtsOwedToUser.map((s, i) => (
                                    <div key={`owed-to-${i}`} className="flex items-center">
                                        ...
                                        <div className="ml-auto font-medium text-green-500">{formatCurrency(s.amount)}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                 ...
                 {balances.settlements.length > 0 && (
                    <Button className="w-full mt-6" disabled>Settle Up</Button>
                )}
            </CardContent>
        </Card>
    );
